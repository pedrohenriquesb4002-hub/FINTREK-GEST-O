...
module.exports = async function handler(req, res) {
  ...
  if (!email || !pass) {
    return res.json({
      ok: false,
      err: 'Email e senha obrigatórios.'
    });
  }
  ...
  try {
    ...
    if (action === 'register') {
      ...
      if (exists.rows.length > 0) {
        return res.json({
          ok: false,
          err: 'Este email já está cadastrado.'
        });
      }
      ...
      await client.query(
        'INSERT INTO ft_sessions (token, user_id) VALUES ($1, $2)',
        [token, user.id]
      );
    ...
    if (action === 'login') {
      ...
      const result = await client.query(
        'SELECT * FROM ft_users WHERE email = $1',
        [email]
      );
      ...
      if (user.pass_hash !== hashPass(pass)) {
        return res.json({
          ok: false,
          err: 'Senha incorreta.'
        });
